...
sortedLanguages.forEach(([name, val]) => {
  const percentage = (val.size / totalLanguageSize) * 100;
  const width = (percentage / 100) * topBarWidth;

  topBar += `
    <rect x="${topBarXStart + offset}" y="${topBarY}" 
      width="${width}" 
      height="${topBarHeight}" 
      fill="${val.color}" rx="2"/>
  `;
...
// 🔥 Lista moderna y proporcional
let languageList = "";
const listYStart = topBarY + topBarHeight + 20;
const listSpacing = 28; 
let yPos = listYStart;
...
<svg width="500" height="${height}" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="30" y="${topBarY - 10}" class="section">Lenguajes más usados</text>
